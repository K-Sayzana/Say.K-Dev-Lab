...
/*
Write a C++ program that permits users to enter the following information about your small company’s 4 employees, sorts the information in ascending ID number, and then writes the sorted information to a binary file:
       ID No. 	Sex(M/F) 	Hourly-Wage 	Years-with-the-Company
      After the records are stored successfully, 
write a program that reads the file created one record at a time, asks for the number of hours each employee worked each month, and calculates and displays each employee’s total pay for the month
...
void sortByID(employee e[]) {// bubble sort
    for(int i=0; i<3; i++) {
        for(int j=0; j<9; j++) {
            if(e[j].ID>e[j+1].ID) {
                employee temp = e[j];
...
void writeSorted(string fileName, employee e[]){
    ...
    for(int i=0; i<10; i++){
        ...
        file.write((char*)&e[i], sizeof(e[i])); // cast size? , 
    }
    file.close();
}
